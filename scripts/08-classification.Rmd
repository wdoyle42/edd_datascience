...
```{r}
za_sum<-za_train%>%group_by(grateful,student,poor)%>%summarize(mean_pizza=mean(got_pizza))
...
pred_table<-prop.table(table(za_train$pred_lm_out,za_train$got_pizza))
...
```{r}
confusionMatrix(za_train$pred_lm_out,za_train$got_pizza)
```
...
## How to think about classifiers
