...
za_train<-za_train%>%
  ...
  mutate(pred_lm_out=ifelse(pred_lm>=.5,1,0)) ## Create predictied classification, 1 if above .5
```
...
```{r}
confusionMatrix(as_factor(za_train$got_pizza),as_factor(za_train$pred_lm_out))
```
...
```{r}
confusionMatrix(data=as_factor(za_train$pred_logit_out),reference=as_factor(za_train$got_pizza))
```
...
confusionMatrix(data=as_factor(za_test$pred_logit_out),reference=as_factor(za_test$got_pizza))
```
