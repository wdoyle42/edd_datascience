...
with(els, prop.table(table(bypared)))

## Simplify dataset by dropping missing parental education cases
els<-els%>%filter(is.na(bypared)==FALSE)

#Conditional Means
els%>%group_by(bypared)%>%
  summarize(mean(bynels2m,na.rm=TRUE))%>%
  slice(1:4)

#Regression
factor_mod<-lm(bynels2m~bypared,data=els);summary(factor_mod)
```

The reference for the model (the excluded category) is parents with less than a high school education. Each coefficient is in comparison to this category, indicating how much higher or lower test scores are among students with parents at different levels of education.  
...
g2<-ggplot(data=filter(els,is.na(bypared)==FALSE),
            aes(x=byses1,y=bynels2m,
                color=bypared #notice the color option
                ))
...
g4<-ggplot(data=hypo_data,
           aes(x=byses1,
               y=pred,
               color=fct_reorder(f=bypared,-pred))) #notice color
...
To show this in the data we can break it out for every type of child age grouping. This takes advantage of faceting.  
...
g5<-ggplot(els,aes(x=byses1,y=bynels2m, color=bypared))
g5<-g5+geom_point(alpha=.5,size=.1)
g5<-g5+geom_smooth(method="lm",color="black")
g5<-g5+facet_wrap(~bypared,ncol=2)
